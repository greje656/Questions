...
We started writing a Stingray plugin that supported simple scene reflection into Arnold. We implemented a custom Arnold display driver which allowed us to forward the linear rendered tiles directly into the Stingray viewport where they would then get tonemapped with the same tone mapper as Stingray (minimizing as many potential source of error as possible).
